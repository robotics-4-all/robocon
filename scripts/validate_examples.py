...
def validate_examples():
    ...
    table = Table(title="RoboConnect Examples Validation")
    ...
    console.print(Panel("[bold blue]Starting validation of all examples...[/bold blue]"))

    # Try to find roboconnect executable
    roboconnect_bin = 'roboconnect'
    # Check if we are in a venv and if roboconnect is there
    ...
    for filename in rbr_files:
        ...
        try:
            # Run roboconnect validate command
            result = subprocess.run(
                [roboconnect_bin, 'validate', filepath],
                capture_output=True,
                text=True,
                check=False
            )
