...
class NavRobotBridgeNode(Node):
    def __init__(self):
        super().__init__('NavRobot_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge(self, '/tf', TFMessage, self.broker_type, 'robot/amcl/tf', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from sensor_msgs.msg import LaserScan
        br = B2RTopicBridge(self, '/scan', LaserScan, self.broker_type, 'robot/amcl/scan', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from nav_msgs.msg import Odometry
        br = B2RTopicBridge(self, '/odom', Odometry, self.broker_type, 'robot/amcl/odom', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from tf2_msgs.msg import TFMessage
        br = B2RTopicBridge(self, '/tf', TFMessage, self.broker_type, 'robot/amcl/tf', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from tf2_msgs.msg import TFMessage
        br = B2RTopicBridge(self, '/tf_static', TFMessage, self.broker_type, 'robot/amcl/tf_static', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from geometry_msgs.msg import PoseWithCovarianceStamped
        br = B2RTopicBridge(self, '/initialpose', PoseWithCovarianceStamped, self.broker_type, 'robot/amcl/initialpose', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from std_srvs.srv import Empty
        br = B2RServiceBridge(self, '/global_localization', Empty, self.broker_type, 'robot/amcl/global_localization', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## <-----------------------------------------------------------------------
