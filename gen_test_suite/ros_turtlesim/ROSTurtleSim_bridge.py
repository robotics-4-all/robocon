...
class ROSTurtleSimBridgeNode:
    def __init__(self):
        rospy.init_node('ROSTurtleSim_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge('/turtle1/pose', Pose, self.broker_type, 'turtle1/pose', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge R2B
        from turtlesim.msg import Color
        br = R2BTopicBridge('/turtle1/color_sensor', Color, self.broker_type, 'turtle1/color_sensor', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from geometry_msgs.msg import Twist
        br = B2RTopicBridge('/turtle1/cmd_vel', Twist, self.broker_type, 'turtle1/cmd_vel', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from std_srvs.srv import Empty
        br = B2RServiceBridge('/reset', Empty, self.broker_type, 'turtle1/reset', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from turtlesim.srv import Spawn
        br = B2RServiceBridge('/spawn', Spawn, self.broker_type, 'turtle1/spawn', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from turtlesim.srv import Kill
        br = B2RServiceBridge('/kill', Kill, self.broker_type, 'turtle1/kill', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from turtlesim.srv import SetPen
        br = B2RServiceBridge('/turtle1/set_pen', SetPen, self.broker_type, 'turtle1/set_pen', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from turtlesim.srv import TeleportAbsolute
        br = B2RServiceBridge('/turtle1/teleport_absolute', TeleportAbsolute, self.broker_type, 'turtle1/teleport_absolute', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from turtlesim.srv import TeleportRelative
        br = B2RServiceBridge('/turtle1/teleport_relative', TeleportRelative, self.broker_type, 'turtle1/teleport_relative', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
