...
class MappingRobotBridgeNode(Node):
    def __init__(self):
        super().__init__('MappingRobot_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge(self, '/map', OccupancyGrid, self.broker_type, 'slam/map', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge R2B
        from tf2_msgs.msg import TFMessage
        br = R2BTopicBridge(self, '/tf', TFMessage, self.broker_type, 'slam/tf', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from sensor_msgs.msg import LaserScan
        br = B2RTopicBridge(self, '/scan', LaserScan, self.broker_type, 'slam/scan', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from nav_msgs.msg import Odometry
        br = B2RTopicBridge(self, '/odom', Odometry, self.broker_type, 'slam/odom', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from tf2_msgs.msg import TFMessage
        br = B2RTopicBridge(self, '/tf', TFMessage, self.broker_type, 'slam/tf', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from slam_toolbox.srv import SaveMap
        br = B2RServiceBridge(self, '/slam_toolbox/save_map', SaveMap, self.broker_type, 'slam/save_map', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from slam_toolbox.srv import SerializePoseGraph
        br = B2RServiceBridge(self, '/slam_toolbox/serialize_map', SerializePoseGraph, self.broker_type, 'slam/serialize_map', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## <-----------------------------------------------------------------------
