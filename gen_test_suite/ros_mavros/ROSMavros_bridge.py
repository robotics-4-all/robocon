...
class ROSMavrosBridgeNode:
    def __init__(self):
        rospy.init_node('ROSMavros_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge('/mavros/local_position/pose', PoseStamped, self.broker_type, 'drone1/local_pose', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge R2B
        from sensor_msgs.msg import BatteryState
        br = R2BTopicBridge('/mavros/battery', BatteryState, self.broker_type, 'drone1/battery', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge R2B
        from mavros_msgs.msg import State
        br = R2BTopicBridge('/mavros/state', State, self.broker_type, 'drone1/state', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge R2B
        from sensor_msgs.msg import Imu
        br = R2BTopicBridge('/mavros/imu/data', Imu, self.broker_type, 'drone1/imu', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from mavros_msgs.srv import CommandBool
        br = B2RServiceBridge('/mavros/cmd/arming', CommandBool, self.broker_type, 'drone1/arm', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from mavros_msgs.srv import SetMode
        br = B2RServiceBridge('/mavros/set_mode', SetMode, self.broker_type, 'drone1/set_mode', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from mavros_msgs.srv import CommandTOL
        br = B2RServiceBridge('/mavros/cmd/takeoff', CommandTOL, self.broker_type, 'drone1/takeoff', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from mavros_msgs.srv import CommandTOL
        br = B2RServiceBridge('/mavros/cmd/land', CommandTOL, self.broker_type, 'drone1/land', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
