...
class PerceptionRobotBridgeNode(Node):
    def __init__(self):
        super().__init__('PerceptionRobot_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge(self, '/vision/detections', Detection2DArray, self.broker_type, 'vision/detections', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from sensor_msgs.msg import Image
        br = B2RTopicBridge(self, '/camera/image_raw', Image, self.broker_type, 'vision/image_raw', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
...
def main():
    rclpy.init()
    node = PerceptionRobotBridgeNode()
    rclpy.spin(node)
    node.destroy_node()
    rclpy.shutdown()
