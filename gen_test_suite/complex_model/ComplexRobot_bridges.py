...
class ComplexRobotBridgeNode(Node):
    def __init__(self):
        super().__init__('ComplexRobot_bridge')
        self.br_list = []
        self._init_broker()
        self._init_bridges()
    ...
    def _init_bridges(self):
        ...
        br = R2BTopicBridge(self, '/cmd_vel', Twist, self.broker_type, 'robot/cmd_vel', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from nav_msgs.msg import Odometry
        br = B2RTopicBridge(self, '/odom', Odometry, self.broker_type, 'robot/odom', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from sensor_msgs.msg import LaserScan
        br = B2RTopicBridge(self, '/scan', LaserScan, self.broker_type, 'robot/scan', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## Topic Bridge B2R
        from tf2_msgs.msg import TFMessage
        br = B2RTopicBridge(self, '/tf', TFMessage, self.broker_type, 'robot/tf', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## RPC Bridge B2R
        from nav_msgs.srv import GetMap
        br = B2RServiceBridge(self, '/get_map', GetMap, self.broker_type, 'robot/get_map', self.conn_params)
        self.br_list.append(br)
        ## <-----------------------------------------------------------------------
        ## <-----------------------------------------------------------------------
        ## <-----------------------------------------------------------------------
